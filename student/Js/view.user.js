document.addEventListener("DOMContentLoaded", async function () {
    ...
    if (token) {
        try {
            ...
            // Mostrar el username en el div info-user
            const infoUserDiv = document.querySelector('.info-user p');
            if (infoUserDiv) {
                infoUserDiv.textContent = username;
            }
        ...
        const infoUserDiv = document.querySelector('.info-user p');
        if (infoUserDiv) {
            infoUserDiv.textContent = 'Token no encontrado';
        }
    }
});